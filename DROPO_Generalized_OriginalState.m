...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%-----------------Sim parameters---------%%%%%%%%%%%%%%%%%%%%%%%%
Nw = 2000;             % slicing number for time
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%---------Pumping scheme--------%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Ip0 = 18e-3*3.4*1.1*28*1.55; Ip00 = 1*Ip0; % 00 for inital pulse seeding
Is0 = 0;             Is00 =1*Ip0;
Ii0 = 0;             Ii00 = 0*Ip0;
...
for qq = 1:length(AP)
    for jj = 1:length(DKK)
        detunes = alphas*L*AP(qq);
        detunep = lams0/lamp0*detunes;

        detunei = lamp0/lami0*detunep;

        Nrt = 500*4;                     % roundtrip number
        dk = DKK(jj)*pi/L; % Phase mismatch

        name = strcat('FlippedSigns_beta_neg_delta_p',num2str(detunep./alphap./L),'dk_',num2str(dk./pi*L),'pi___','Ip0_',num2str(Ip0));
        ...
        beta1_p = 1/(c/2.523);                              
        beta1_s = 1/(c/2.523);
        beta1_i = 1/(c/2.523);         % Compensation given in units of s for GVM and s^2 for GVD
        beta_offset1 = beta1_p-beta1_s;      beta1_comp_p = -1*(beta1_p-beta1_p).*L;
        beta_offset2 = beta1_p-beta1_s;      beta1_comp_s = -1*(beta1_p-beta1_s).*L;
        beta_offset3 = beta1_p-beta1_s;      beta1_comp_i = -1*(beta1_p-beta1_i).*L;
        %%%%%%%---------GVD-------%%%%%%%%% %%%%%%%---------GVD Compensation-------%%%%%%%%%
        beta2_p =  281e-27;                   beta2_comp_p = (-281e-27*0).*L;
        beta2_s =  213e-27;                   beta2_comp_s = (-213e-27*0).*L;
        beta2_i =  -1096e-27./1;              beta2_comp_i = (1096e-27*0).*L;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        Dcomp_p = 1.*exp(1j*(beta1_comp_p.*w + beta2_comp_p/2.*w.^2)); 
        Dcomp_s = 1.*exp(1j*(beta1_comp_s.*w + beta2_comp_s/2.*w.^2));
        Dcomp_i = 1.*exp(1j*(beta1_comp_i.*w + beta2_comp_i/2.*w.^2)); 
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
